...
internals.Listener.prototype._add = function (ws, req) {
    ...
    ws.once('close', async (code, message) => {
        ...
        socket._subscriptionsWithPaths = {};
...
internals.Listener.prototype._publish = function (path, _update, options) {
    ...
    const each = async (socket, queryParams) => {       // Filter on path if has parameters

        let update = _update;
        queryParams.forEach((params, idx) => {
            Object.assign(match.params, params);
        });
        if (route.filter) {
            try {
...
internals.Listener.prototype._subscribe = async function (path, socket) {
    ...
    const origPath = path;
    const queryDelimiterPos = path.indexOf('?');
    if (queryDelimiterPos !== -1) {
        const queryParamsStr = path.substring(queryDelimiterPos);
        queryParams = QueryString.parse(queryParamsStr);
        path = path.substring(0, queryDelimiterPos);
    }

    if (socket._subscriptionsWithPaths[origPath]) { 
        // one client can't open two subs with the same path/query params
        return;
    }
    ...
    // assign to match.params, handlling comma separated param values
    if (queryParams) {
        Object.keys(queryParams).forEach((key, idx) => {
            match.params[key] = queryParams[key].split(',');
        });
    }
    ...
    socket._subscriptionsWithPaths[origPath] = match.route.subscribers;
};
...
internals.Subscribers.prototype._forEachSubscriber = async function (path, options, each) {
    ...
    const itemize = async (item) => {
        ...
        if (item &&         // check item not removed
            ...
            await each(item.socket, item.params);
        }
    };
