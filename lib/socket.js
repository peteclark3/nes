...
exports = module.exports = internals.Socket = function (ws, req, listener) {
    ...
    if (this._listener._settings.auth &&
        ...
        this.auth._timeout = setTimeout(() => this.disconnect(), this._listener._settings.auth.timeout);
    }
...
internals.Socket.prototype._lifecycle = function (request) {
...
internals.Socket.prototype._processHello = async function (request) {
    ...
    if (request.auth) {
        const auth = { credentials: null, artifacts: null };
        ...
        if (config.type === 'direct') {
            ...
            auth.credentials = res.result.credentials;
            auth.artifacts = res.result.artifacts;
        }
        else {
            try {
        ...
        this._setCredentials(auth.credentials, auth.artifacts);
    }
...
internals.Socket.prototype._authenticate = async function () {
